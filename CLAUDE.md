...
- 大問番号の表記は実際の試験冊子に合わせ、JS定数 `QUESTION_NO_STYLE`(大学ごと。正本はここ)で切り替える。`daimon`=「第1問」(東大)/`boxed`=四角い枠付き数字(京大。シートでは左寄せ)。未登録の大学は「第◯問」。適用箇所はアーカイブの大問一覧・タグ検索の結果カード・詳細見出し・問題文シートの4箇所。タグ検索の大問番号フィルタチップと管理画面は大学横断のため「第◯問」のまま
