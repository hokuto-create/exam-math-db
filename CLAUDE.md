# CLAUDE.md — 大学入試 過去問データベース

## プロジェクト概要

大学入試の過去問を「大学 × 年度 × 大問」単位でレコード化した**過去問データベース**サイト(サイト名: 「大学入試 過去問DB」。将来の理科(物理・化学・地学)拡張を見据えて科目を冠しない名称にしている。現在の収録科目は数学のみ)。メインは「大学 → 年度・試験区分 → 大問」と辿るアーカイブ閲覧で、**単元タグ**と**解法キータグ**の二軸で逆引きするタグ検索(辞書的な使い方)をサブ機能として持つ。
...
- **京都大学**: 「試験問題等の利用について」(https://www.kyoto-u.ac.jp/ja/admissions/undergrad/past-eq/copyright-policy)により、京大に著作権が帰属する入試問題等(第三者著作物を除く)の複製・譲渡および自動公衆送信(Web掲載)は、条項の遵守を条件に**事前承認なしで許可**。ただし以下を厳守:
  ...
  2. **出典明示・改変明示**: 転記した `problem_text` の末尾に必ず出典行を入れる。書式: `出典:京都大学 ◯◯年度 入学試験問題 数学(理系/文系)第◯問(原本より数式の組版を変更して転載)`
...
create table problems (
  ...
  points      int,                          -- 配点(任意。試験冊子に明記される大学のみ)
...
- **書き方(読みやすさのルール)**: 表示は `white-space: pre-wrap` なので改行がそのまま出る。段落の間には**空行を1行**入れる/方針は小問ごと・手順ごと(①②③)に短く区切る/1行に収まらない長い式は `$$...$$` で別行立てにする(スマホ幅での途中折り返しを防ぐ)
- **記法は高校数学の慣例に合わせる**: 順列・組合せは `{}_n\mathrm{P}_r` / `{}_n\mathrm{C}_r`(`\binom` は使わない)。そのほかも教科書・試験冊子で使われる記法を優先する
...
- 基本情報(大学・年度・大問・試験区分・公式PDFリンク)。戻るボタンは遷移元(アーカイブの大学ページ/タグ検索)へ戻す
- 単元タグ・解法キータグは講評コメントの**下**にまとめて表示する
- タグの下に「類題」セクション: 単元タグと解法キーの**両方**が1つ以上重なる他の問題を関連度順(解法キー一致数×2+単元一致数、同点は年度降順)に最大5件カード表示。該当0件ならセクションごと非表示(データが増えると自動的に現れる)。カードから類題の詳細へ遷移でき、戻り先は元の遷移元を引き継ぐ
- 問題文: `problem_text` があれば試験冊子風シート(明朝体・大問番号見出し・大学/年度メタ)で表示(HTMLエスケープ+改行保持、MathJax対応)。空なら非表示。末尾の「出典:…」行は本文から自動分離してシート下部に小さく表示する
